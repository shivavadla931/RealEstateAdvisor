...
# LOAD DATASET
# ---------------------------------------------------------------
DATA_PATH = "https://drive.google.com/uc?export=download&id=1ocXmSTnawuKzJNHZmEmuWkMFsVt9nLfh"
df_raw = None
try:
    df_raw = pd.read_csv(DATA_PATH)
except Exception as e:
    st.error(f"Could not find dataset at `{DATA_PATH}`. Place `india_housing_prices.csv` in the `data/` folder. Details: {e}")
    df_raw = None
